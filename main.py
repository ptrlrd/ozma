...
custom_ocean_fishing = discord.PartialEmoji(name="Fisher", id=554478937028427796)
custom_treasuremap = discord.PartialEmoji(name="treasuremap", id=679192676486086658)
custom_blue_mage = discord.PartialEmoji(name="BlueMage", id=554479007287214090)
custom_savage_raiding = discord.PartialEmoji(name="savage", id=638832705106346036)
custom_extreme_trials_raiding = discord.PartialEmoji(name="extreme", id=638832720201515049)
custom_ultimate_raiding = discord.PartialEmoji(name="ultimate", id=638832734231592965)
custom_unreal_raiding = discord.PartialEmoji(name="unreal", id=638832750559756329)
custom_pvp = discord.PartialEmoji(name="pvp", id=638832685346979871)
custom_unsynced_content = discord.PartialEmoji(name="unsync", id=979082924022431784)
...
custom_tools = discord.PartialEmoji(name="toolsdiscussion", id=1013634974404051044)
custom_currentevents = discord.PartialEmoji(name="currentevents", id=719347909048402010)

custom_orange_star = discord.PartialEmoji(name="orange_star", id=874102281002430545)
custom_pink_star = discord.PartialEmoji(name="pink_star", id=874102281069555752)
custom_purple_star = discord.PartialEmoji(name="purple_star", id=874102281094717460)
custom_teal_star = discord.PartialEmoji(name="teal_star", id=874102281002426428)
custom_green_star = discord.PartialEmoji(name="green_star", id=983910897712001094)
custom_yellow_star = discord.PartialEmoji(name="yellow_star", id=983911288151347202)
custom_black_star = discord.PartialEmoji(name="black_star", id=983911288310730762)
custom_white_star = discord.PartialEmoji(name="white_star", id=983911288176517141)
    ...
    def load_message_ids(self):
        ...
        if os.path.exists(data_file):
            with open(data_file, "r") as f:
                ...
                print(f"Loaded message_id_store: {message_id_store}")  # Add this line
...
def save_message_ids(client):
    ...
    with open(data_file, "w") as f:
        ...
        print(f"Saved message_id_store: {client.message_id_store}")  # Add this line


role_categories = {
    "XIV Roles": ["Tank", "Healer", "DPS", "Crafter", "Gatherer", "Ocean Fishing", "Maps", "Blue Mage",
...
class RoleButton(discord.ui.Button):
    def __init__(self, *, role, label=None, style=None, emoji=None, custom_id=None):
        self.role = role
        ...
        super().__init__(label=label, style=discord.ButtonStyle.secondary, emoji=emoji, custom_id=custom_id)
            ...
            if self.role in updated_member.roles:
                try:
                    ...
                    await interaction.response.send_message(
                        f"Successfully removed the {self.role.name} role from {updated_member.mention}.",
                ...
                try:
                    ...
                    await interaction.response.send_message(
                        f"Successfully added the {self.role.name} role to {updated_member.mention}.", ephemeral=True,
                        delete_after=30)
                except Exception as e:
                    await send_error_message(interaction, e)
        except Exception as e:
...
class ChannelSelectionView(discord.ui.View):
    ...
    async def channel_select(self, select: discord.ui.Select, interaction: discord.Interaction):
        ...
        await interaction.response.send_message("Channel selected successfully!", ephemeral=True, delete_after=30)
        self.stop()
...
#
#     async def interaction_check(self, interaction: discord.Interaction) -> bool:
#         booster_role = discord.utils.get(interaction.guild.roles, name="Nitro Booster")
#         if booster_role is None:
#             return False  # Don't show the view if the Nitro Booster role doesn't exist
#         else:
...
#                     await interaction.response.send_message(
#                         f"Successfully removed the {role.name} role from {updated_member.mention}.", ephemeral=True,
#                         delete_after=30)
#                 except Exception as e:
#                     await send_error_message(interaction, e)
#             else:
...
#                     await interaction.response.send_message(
#                         f"Successfully added the {role.name} role to {updated_member.mention}.", ephemeral=True,
#                         delete_after=30)
#                 except Exception as e:
#                     await send_error_message(interaction, e)
#         else:
#             await interaction.response.send_message(f"Role '{role_name}' not found on the server.", ephemeral=True,
#                                                     delete_after=30)
...
async def send_error_message(interaction: discord.Interaction, error):
    ...
    if interaction.command is not None:
        ...
        print(f"An error occurred in command execution by {interaction.user.mention} in {interaction.command}.")
...
async def search_character(first_name: str, last_name: str, server: str):
    async with aiohttp.ClientSession() as session:
        ...
        completed_url = f"{search_url}?name={first_name} {last_name}&server={server}"
...
async def on_interaction(interaction: discord.Interaction):
    ...
    if interaction.type == discord.InteractionType.component:
        ...
        # If you have other types of interactions (e.g. slash commands), handle them here
        pass
...
async def lookup_character(interaction: discord.Interaction, first_name: str, last_name: str, server: str):
    try:
        ...
        if character_data:
            ...
            if results:
                ...
                main_class = character_details["Character"]["ActiveClassJob"]["Name"]
                ...
                await interaction.edit_original_response(content="No character found.")
        else:
            await interaction.edit_original_response(content="An error occurred while looking up the character.")
    except Exception as e:
        await interaction.edit_original_response(content="An error occurred while looking up the character.")
        await send_error_message(interaction, e)
...
async def post_role_buttons(interaction: discord.Interaction, channel: discord.TextChannel):
    ...
    try:
        ...
        await interaction.response.send_message("Role buttons posted in the target channel.", ephemeral=True,
                                                delete_after=30)
